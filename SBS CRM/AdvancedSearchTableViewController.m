...
@synthesize cellType1 = _cellType1;
@synthesize cellContact = _cellContact;
@synthesize lblType1CellDetail = _lblType1CellDetail;
@synthesize cellType2 = _cellType2;
@synthesize cellDueDateFrom = _cellDueDateFrom;
@synthesize cellDueDateTo = _cellDueDateTo;
@synthesize cellCreatedDateFrom = _cellCreatedDateFrom;
@synthesize cellCreatedDateTo = _cellCreatedDateTo;
@synthesize segStatus = _segStatus;
@synthesize txtTitle = _txtTitle;
@synthesize txtComments = _txtComments;
@synthesize cellInternalContact = _cellInternalContact;
@synthesize cellNoOfRecords = _cellNoOfRecords;
@synthesize segOrderBy = _segOrderBy;
...
- (void)viewDidLoad
{
    ...
    self.cellCompanySite.detailTextLabel.text = self.cosSiteName;
    if (self.fullName)
        self.cellContact.detailTextLabel.text = self.fullName;
    ...
    switch (indexPath.section) {
        case 0:
            if (indexPath.row == 1) {
                ...
                if([self fetchContacts:self.companySiteID])
                {
                    item = self.cellContact.detailTextLabel.text;
        ...
        case 1:
            if (indexPath.row == 0) {
                ...
                {
                    item = self.cellType1.detailTextLabel.text;
            ...
            else
            {
                if ([self fetchEventType2s]) {
                    item = self.cellType2.detailTextLabel.text;
        ...
        case 3:
            if (indexPath.row == 0) {
                item = self.cellCreatedDateFrom.detailTextLabel.text;
                sourceCellIdentifier = @"cellCreatedDateFrom.detailTextLabel.text";
                [self performSegueWithIdentifier:@"toDateTimePicker" sender:self];
            }
            else {
                item = self.cellCreatedDateTo.detailTextLabel.text;
        ...
        case 6: 
            ...
            if ([self fetchContacts:[NSString stringWithFormat:@"%d", appCompanySiteID]]) {
                item = self.cellInternalContact.textLabel.text;
                sourceCellIdentifier = @"cellInternalContact.textLabel.text";
                [self performSegueWithIdentifier:@"toLookUpTableView" sender:self];
            }
            break;
        case 7: 
            item = self.cellNoOfRecords.textLabel.text;
...
- (void)prepareForSegue:(UIStoryboardSegue *)segue sender:(id)sender
{
    if ([segue.identifier isEqualToString:@"toLookUpTableView"]) {
        ...
        if (![self.cellDueDateFrom.detailTextLabel.text isEqualToString:@"All"]) 
            dueDateFrom = [dfToString stringFromDate:[dfToDate dateFromString:self.cellDueDateFrom.detailTextLabel.text]];
        if (![self.cellDueDateTo.detailTextLabel.text isEqualToString:@"All"]) 
            dueDateTo = [dfToString stringFromDate:[dfToDate dateFromString:self.cellDueDateTo.detailTextLabel.text]];
        if (![self.cellCreatedDateFrom.detailTextLabel.text isEqualToString:@"All"]) 
            createdDateFrom = [dfToString stringFromDate:[dfToDate dateFromString:self.cellCreatedDateFrom.detailTextLabel.text]];
        if (![self.cellCreatedDateTo.detailTextLabel.text isEqualToString:@"All"]) 
            createdDateTo = [dfToString stringFromDate:[dfToDate dateFromString:self.cellCreatedDateTo.detailTextLabel.text]];
        ...
        if (self.segOrderBy.selectedSegmentIndex == 1)
            eventListViewController.orderByCreatedDate = YES;
        
        eventListViewController.company = self.company;
        
        eventListViewController.advancedURL = [appURL stringByAppendingFormat:@"/service1.asmx/advancedEventSearchABL?companySiteID=%@&contactID=%@&eventType=%@&eventType2=%@&dueDateFrom=%@&dueDateTo=%@&createdDateFrom=%@&createdDateTo=%@&status=%d&title=%@&comments=%@&internalContactID=%@&noOfRecords=%@&orderBy=%d", self.companySiteID, self.contactID, eventTypeID, eventType2ID, dueDateFrom, dueDateTo, createdDateFrom, createdDateTo, self.segStatus.selectedSegmentIndex, self.txtTitle.text, self.txtComments.text, internalContactID, self.cellNoOfRecords.textLabel.text, self.segOrderBy.selectedSegmentIndex];
...
- (void) lookUpTableViewController: (LookUpTableViewController  *) controller didSelectItem:(NSInteger *)returnedRow withSourceCellIdentifier:(NSString *)returnedSourceCellIdentifier
{
    ...
    if ([returnedSourceCellIdentifier isEqualToString:@"cellContact.detailTextLabel.text"]) {
        if (returnedRow == 0)
            ...
            self.contactID = [(ContactSearch *)[contactArray objectAtIndex:((int)returnedRow - 1)] contactID];
    }
...
-(void)pickerViewController:(PickerViewController *)controller didSelectItem: (NSString *)returnedItem withSourceCellIdentifier:(NSString *)returnedSourceCellIdentifier
{    
    //set the value to the correct cell using the source identifier
...
- (BOOL)fetchContacts:(NSString *)searchCompanySiteID
{
    ...
    for (DDXMLElement *element in nodes) { 
        ...
        self.fullName = [nameArray componentsJoinedByString:@"\n"];
        
        [itemArray addObject:self.fullName];
...
{    
    ...
    for (NSDictionary *tempEventDict in eventTypeArray) {   
        if ([tempEventDict objectForKey:self.cellType1.detailTextLabel.text])
            selectedEventTypeID = [tempEventDict objectForKey:self.cellType1.detailTextLabel.text];
    }
...
{    
    ...
    if (![self.cellDueDateFrom.detailTextLabel.text isEqualToString:@"All"] && ![self.cellDueDateTo.detailTextLabel.text isEqualToString:@"All"]) {
        
        if([[dfToDate dateFromString:self.cellDueDateFrom.detailTextLabel.text] compare: [dfToDate dateFromString:self.cellDueDateTo.detailTextLabel.text]] == NSOrderedDescending) {
    ...
    if (![self.cellCreatedDateFrom.detailTextLabel.text isEqualToString:@"All"] && ![self.cellCreatedDateTo.detailTextLabel.text isEqualToString:@"All"]) {
        
        if([[dfToDate dateFromString:self.cellCreatedDateFrom.detailTextLabel.text] compare: [dfToDate dateFromString:self.cellCreatedDateTo.detailTextLabel.text]] == NSOrderedDescending) {
