...
@synthesize advancedURL = _advancedURL;
@synthesize orderByCreatedDate = _orderByCreatedDate;
...
- (void)getData
{
    ...
    NSLog(@"advanced url: %@", self.advancedURL);
    
    //depending on the dom document, set the appropriate URL
    if (self.advancedURL) {
        NSLog(@"company: %@",self.company.coaCompanyName);
        if (![getContactsDom fetchXMLWithURL:self.advancedURL])
        {[connectionErrorAlert show]; return;} 
        
    }
    else if (self.contact) { // if there is a contact ID perform the search by contact
        
        if (![getContactsDom fetchXMLWithURL:[appURL stringByAppendingFormat:@"/service1.asmx/searchEventsByContactABL?searchContactID=%@", self.contact.contactID]]) {
            [connectionErrorAlert show]; 
            return;
        }
        
    }
    else if (self.company) { // if there is a company site id perform the search by company site
        
        if (![getContactsDom fetchXMLWithURL:[appURL stringByAppendingFormat:@"/service1.asmx/searchEventsByCompanyABL?searchCompanySiteID=%@", self.company.companySiteID]]) {
            [connectionErrorAlert show]; 
            return;
        }
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    cell.siteNameDesc.text = [self.company.cosSiteName stringByAppendingFormat:@" - %@", self.company.cosDescription];
...
- (void)prepareForSegue:(UIStoryboardSegue *)segue sender:(id)sender
{
    ...
    if ([segue.identifier isEqualToString:@"toEventDetails"]) {
        ...
        if (self.contact)
            detailViewController.contact = self.contact;
